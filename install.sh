...
###############################################################################
# TaishanPi SDK Universal Install Script
# For: Ubuntu 22.04 LTS (Jammy Jellyfish)
# Usage:
#   Android: curl -fsSL <URL>/install.sh | bash -s -- -b android14/tspi-3-260416
#   Linux:   curl -fsSL <URL>/install.sh | bash -s -- -b linux/tspi-3-260402
...
REPO_URL="https://cnb.cool/jlckfb/git-repo"
REPO_REV="main"
...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -b <branch>   Manifest branch (required), e.g. android14/tspi-3-260416"
    echo "  -d <dir>      SDK install directory (optional, auto-generated by default)"
...
parse_args() {
    ...
    if [[ -z "$MANIFEST_BRANCH" ]]; then
        log_error "Missing required option: -b <branch>"
        echo ""
        usage
    fi

    case "$MANIFEST_BRANCH" in
        android*) SDK_TYPE="android" ;;
        linux*)   SDK_TYPE="linux" ;;
        *)
            log_error "Unknown SDK type from branch: $MANIFEST_BRANCH"
            log_error "Branch must start with 'android' or 'linux'"
            exit 1
            ;;
    esac
    ...
    if [[ -z "$SDK_DIR" ]]; then
        case "$SDK_TYPE" in
            android) SDK_DIR="$PWD/TaishanPi-3-Android14" ;;
            linux)   SDK_DIR="$PWD/TaishanPi-3-Linux" ;;
        esac
    fi
}
...
install_dependencies() {
    ...
    if [[ "$SDK_TYPE" == "linux" ]]; then
        PACKAGES+=(
            crossbuild-essential-arm64 whiptail
            binfmt-support qemu-user-static
            gcc-aarch64-linux-gnu g++-aarch64-linux-gnu
            debootstrap cpio iputils-ping pigz tar aria2 zfs-fuse
            libgtk2.0-dev libxxf86vm1 linux-headers-generic
        )
    fi
...
clone_sdk() {
    log_step "Cloning SDK ($SDK_TYPE)"
    echo ""
    mkdir -p "$SDK_DIR"
    pushd "$SDK_DIR" > /dev/null

    echo -e "${CYAN}  SDK will be installed to: $SDK_DIR${NC}"
...
fetch_lfs_objects() {
    log_step "Fetching Git LFS objects (auto-detect)"
    echo ""

    local lfs_dirs=()
    while IFS= read -r attr_file; do
        lfs_dirs+=("$(dirname "$attr_file")")
    done < <(find "$SDK_DIR" -name ".gitattributes" -exec grep -l "filter=lfs" {} \; 2>/dev/null)

    if [[ ${#lfs_dirs[@]} -eq 0 ]]; then
        log_info "No LFS repositories detected"
        echo ""
        return 0
    fi

    local total=${#lfs_dirs[@]}
    local current=0
    log_info "Found $total LFS repositories"

    for full_path in "${lfs_dirs[@]}"; do
        ((current++))
        local rel_path="${full_path#$SDK_DIR/}"
        ...
        while kill -0 $lfs_pid 2>/dev/null; do
            ...
            if [[ -n "$lfs_status" && "$lfs_status" == *"%"* ]]; then
                printf "\r\033[K  %s LFS (%d/%d) %s: %s [%02d:%02d]" "${lframes[$si]}" "$current" "$total" "$rel_path" "$lfs_status" $((elapsed/60)) $((elapsed%60))
            else
                printf "\r\033[K  %s LFS (%d/%d) %s... [%02d:%02d]" "${lframes[$si]}" "$current" "$total" "$rel_path" $((elapsed/60)) $((elapsed%60))
            fi
            sleep 0.3
        done
        wait $lfs_pid
        ...
        if [[ $lfs_ret -ne 0 ]]; then
            echo -e "${RED}[ERR]${NC} LFS ($current/$total) ${rel_path} ${RED}Failed${NC} ${DIM}($(printf '%02d:%02d' $((elapsed/60)) $((elapsed%60))))${NC}"
            log_warn "LFS pull failed: $rel_path"
        else
            echo -e "${GREEN}[OK]${NC} LFS ($current/$total) ${rel_path} ${DIM}($(printf '%02d:%02d' $((elapsed/60)) $((elapsed%60))))${NC}"
        fi
        popd > /dev/null
    done
...
main() {
    ...
    [[ "$SDK_TYPE" == "linux" ]] && TOTAL_STAGES=8
    local STAGE_NUM=0

    print_box "TaishanPi SDK Installer ($SDK_TYPE)" "$MAGENTA"
    echo -e "${DIM}  Log file: ${LOG_FILE}${NC}"
    echo -e "${DIM}  Branch:   ${MANIFEST_BRANCH}${NC}"
    echo -e "${DIM}  SDK dir:  ${SDK_DIR}${NC}"
    echo ""

    local stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Network Detection"
    ...
    stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: System Check"
    print_box "$CURRENT_STAGE" "$BLUE"
    ...
    stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Install Dependencies"
    print_box "$CURRENT_STAGE" "$BLUE"
    ...
    if [[ "$SDK_TYPE" == "linux" ]]; then
        stage_start=$SECONDS
        ((STAGE_NUM++))
        CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Install live-build"
        print_box "$CURRENT_STAGE" "$BLUE"
        ...
        stage_start=$SECONDS
        ((STAGE_NUM++))
        CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Configure Linux Environment"
        print_box "$CURRENT_STAGE" "$BLUE"
    ...
    stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Setup Repo"
    print_box "$CURRENT_STAGE" "$BLUE"
    ...
    stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Clone SDK"
    print_box "$CURRENT_STAGE" "$BLUE"
    local do_clone=""
    safe_read "Clone SDK now? [y/N]: " do_clone
    echo ""
    if [[ "$do_clone" =~ ^[Yy]$ ]]; then
        clone_sdk || return 1
    else
    ...
    stage_start=$SECONDS
    ((STAGE_NUM++))
    CURRENT_STAGE="Stage ${STAGE_NUM}/${TOTAL_STAGES}: Fetch LFS Objects"
    print_box "$CURRENT_STAGE" "$BLUE"
    if [[ -d "$SDK_DIR" ]]; then
